...
runner.skip(
  ...
  "Toughness mechanic is still being developed — test disabled until role system stabilizes",
  (engine) => {
    const mode = GameModeFactory.getInstance().createMode("classic");
    engine.setGameMode(mode);
    ...
    assertEqual(tough.accumulatedDamage, 50, "Tough player should take half damage");
  }
);
