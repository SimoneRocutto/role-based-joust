...
- on round end, when returning to lobby, players are already marked as ready. It also happens when stopping an ongoing game. This should not occur, players ready state should be reset to not ready if an admin clicks on the button to return to the dashboard main screen. Also, if all players get ready on game end, a new game with the same players should be started.
