...
vi.mock('@/services/socket', () => ({
  socketService: {
    ...
    onPlayerJoined: (cb: Function) => socketEventHandlers.set('player:joined', cb),
    onPlayerReconnected: (cb: Function) => socketEventHandlers.set('player:reconnected', cb),
    onGameTick: (cb: Function) => socketEventHandlers.set('game:tick', cb),
    onPlayerDeath: (cb: Function) => socketEventHandlers.set('player:death', cb),
    ...
    onGameStopped: (cb: Function) => socketEventHandlers.set('game:stopped', cb),
    onVampireBloodlust: (cb: Function) => socketEventHandlers.set('vampire:bloodlust', cb),
    onRoleAssigned: (cb: Function) => socketEventHandlers.set('role:assigned', cb),
    onLobbyUpdate: (cb: Function) => socketEventHandlers.set('lobby:update', cb),
    onCountdown: (cb: Function) => socketEventHandlers.set('game:countdown', cb),
...
describe('useSocket', () => {
  ...
  describe('player:joined event', () => {
    it('stores session data in localStorage on success', () => {
      ...
      expect(localStorage.setItem).toHaveBeenCalledWith('playerId', 'player-123')
    ...
    it('merges player data with existing state', () => {
      ...
      act(() => {
        useGameStore.getState().updatePlayers([{
          ...
          toughness: 100,
          accumulatedDamage: 0,
          statusEffects: [{ type: 'Bloodlust', priority: 50, timeLeft: 5000 }],
        }])
      })
  ...
  describe('player:death event', () => {
    it('sets latest event with death message', () => {
      ...
      expect(useGameStore.getState().latestEvent).toBe('Player #3 Alice eliminated!')
    })
    ...
    it('plays death sound when victim is current player', () => {
      ...
      expect(audioPlayCalls).toContainEqual({ sound: 'death', options: { volume: 0.5 } })
    })
    ...
    it('updates player points from scores', () => {
      ...
      act(() => {
        triggerSocketEvent('round:end', {
          scores: [{ playerId: 'p1', playerName: 'Player 1', playerNumber: 1, score: 50, rank: 1, status: 'winner' }],
        })
      })
  ...
  describe('game:end event', () => {
    it('sets final scores and finished state', () => {
      ...
      const scores = [
        { playerId: 'p1', playerName: 'Winner', playerNumber: 1, score: 100, rank: 1, status: 'champion' },
      ]
    ...
    it('plays victory music', () => {
      ...
      expect(audioPlayMusicCalls).toContainEqual({ track: 'victory', options: { loop: false, volume: 0.6 } })
    })
  })
  ...
  describe('game:countdown event', () => {
    ...
    it('plays countdown beep for seconds 1-3', () => {
      ...
      expect(audioPlayCalls).toContainEqual({ sound: 'countdown-beep', options: { volume: 0.5 } })
    })
    ...
    it('plays go sound on go phase', () => {
      ...
      expect(audioPlayCalls).toContainEqual({ sound: 'countdown-go', options: { volume: 0.7 } })
    })
  })
    ...
    it('updates players in waiting state', () => {
      ...
      act(() => {
        triggerSocketEvent('lobby:update', {
          players: [
            { id: 'p1', name: 'Player 1', number: 1, isAlive: true, isReady: false },
            { id: 'p2', name: 'Player 2', number: 2, isAlive: true, isReady: true },
          ],
        })
      })
  ...
  describe('error event', () => {
    it('sets latest event with error message', () => {
      ...
      expect(useGameStore.getState().latestEvent).toBe('Error: Connection failed')
    })
  })
  ...
  describe('vampire:bloodlust event', () => {
    it('starts heartbeat loop for current player when active', () => {
      ...
      expect(audioLoopCalls).toContainEqual({ sound: 'low-health-heartbeat', options: { volume: 0.6 } })
    })
  ...
  describe('role:assigned event', () => {
    ...
    it('plays role reveal sound', () => {
      ...
      expect(audioPlayCalls).toContainEqual({ sound: 'role-reveal', options: { volume: 0.7 } })
    })
    ...
    it('speaks role description after delay', () => {
      ...
      expect(audioSpeakCalls.some(s => s.includes('You are the Civilian'))).toBe(true)
    })
    ...
    it('includes target info in speech when available', () => {
      ...
      expect(audioSpeakCalls.some(s => s.includes('Your target is Player number 5'))).toBe(true)
    })
  })
})
